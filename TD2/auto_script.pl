...
$cmd = `cat corpusA09_filtre.xml | perl prof/newsegmente.pl -f | sort -u | cut -f1 | sort | uniq > mots.txt`;
print "OK\n";
...
$cmd = `perl prof/filtronc.pl -v successeurs.txt | sort | uniq > filtronc.txt`;
print "OK\n";
...
$cmd = `cat inverse.txt | cut -f1 > lemmes_inverse.txt`;
print "OK\n";
print "*******************************END*********************************\n\n";